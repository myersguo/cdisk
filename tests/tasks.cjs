...
(async () => {
  ...
  try {
    ...
    await page.getByRole("button", { name: "暂停扫描", exact: true }).click();
    await page.getByRole("button", { name: "继续扫描", exact: true }).waitFor();
    const secondPausedCount = await page.locator(".candidate-row").count();
    const secondPausedProgress = await page.locator(".scan-progress strong").innerText();
    await page.waitForTimeout(400);
    assert.equal(await page.locator(".candidate-row").count(), secondPausedCount);
    assert.equal(await page.locator(".scan-progress strong").innerText(), secondPausedProgress);
